...
int getRandomNumber(int min, int max) {
	return rand() % (max - min + 1) + min; // ăĺíĺđŕöč˙ ńëó÷ŕéíîăî ÷čńëŕ ěĺćäó min č max
}
...
int main()
{
	...
	if (v == 1) {
		tile.loadFromFile("ňĺđ.png");
	}

	Sprite till(tile);
	float currentFrame = 0;
	Font fot;
	fot.loadFromFile("COld.ttf");//ďĺđĺäŕĺě ôŕéë řđčôňŕ
	Text text("", fot, 40);//ńîçäŕĺě îáúĺęň ňĺęńň. çŕęčäűâŕĺě â îáúĺęň ňĺęńň ńňđîęó, řđčôň, đŕçěĺđ řđčôňŕ(â ďčęńĺë˙ő)
	Text text1("", fot, 40);//ńîçäŕĺě îáúĺęň ňĺęńň. çŕęčäűâŕĺě â îáúĺęň ňĺęńň ńňđîęó, řđčôň, đŕçěĺđ řđčôňŕ(â ďčęńĺë˙ő)
	text1.setFillColor(Color::Red);//ďîęđŕńčëč ňĺęńň â ęđŕńíűé
	text1.setStyle(Text::Bold);//ćčđíűé č ďîä÷ĺđęíóňűé ňĺęńň. 
	Text text2("", fot, 40);//ńîçäŕĺě îáúĺęň ňĺęńň. çŕęčäűâŕĺě â îáúĺęň ňĺęńň ńňđîęó, řđčôň, đŕçěĺđ řđčôňŕ(â ďčęńĺë˙ő)
	text2.setFillColor(Color::Red);//ďîęđŕńčëč ňĺęńň â ęđŕńíűé
	Text textmenu, textmenu2, textmenu3;

	textmenu.setFont(fot); // Óńňŕíîâęŕ řđčôňŕ äë˙ ďĺđâîăî ňĺęńňîâîăî ýëĺěĺíňŕ
	textmenu.setString("Start Game"); // Óńňŕíîâęŕ ňĺęńňŕ "Start Game"
	textmenu.setCharacterSize(36); // Óńňŕíîâęŕ đŕçěĺđŕ řđčôňŕ
	textmenu.setFillColor(Color::Red); // Óńňŕíîâęŕ öâĺňŕ ňĺęńňŕ
	textmenu.setOutlineColor(Color::Black);
	textmenu.setOutlineThickness(2);
	textmenu.setPosition(150, 200); // Óńňŕíîâęŕ ďîçčöčč ňĺęńňŕ
	...
	srand(time(0));  // Číčöčŕëčçŕöč˙ ăĺíĺđŕňîđŕ ńëó÷ŕéíűő ÷čńĺë íŕ îńíîâĺ ňĺęóůĺăî âđĺěĺíč
	...
	while (window.isOpen())
	{
		...
		if (variable == 0) {
			...
			if (event.type == sf::Event::MouseMoved)
			{
				...
				{
					textmenu.setFillColor(sf::Color::Yellow); // Čçěĺíĺíčĺ öâĺňŕ ďđč íŕâĺäĺíčč ęóđńîđŕ
				}
				else
				{
					textmenu.setFillColor(sf::Color::Red); // Âîçâđŕň čńőîäíîăî öâĺňŕ ďđč óőîäĺ ęóđńîđŕ
				}
				if (textmenu2.getGlobalBounds().contains(sf::Vector2f(event.mouseMove.x, event.mouseMove.y)))
				{
					textmenu2.setFillColor(sf::Color::Yellow); // Čçěĺíĺíčĺ öâĺňŕ ďđč íŕâĺäĺíčč ęóđńîđŕ
				}
				else
				{
					textmenu2.setFillColor(sf::Color::White); // Âîçâđŕň čńőîäíîăî öâĺňŕ ďđč óőîäĺ ęóđńîđŕ
				}
				if (textmenu3.getGlobalBounds().contains(sf::Vector2f(event.mouseMove.x, event.mouseMove.y)))
				{
					textmenu3.setFillColor(sf::Color::Yellow); // Čçěĺíĺíčĺ öâĺňŕ ďđč íŕâĺäĺíčč ęóđńîđŕ
				}
				else
				{
					textmenu3.setFillColor(sf::Color::White); // Âîçâđŕň čńőîäíîăî öâĺňŕ ďđč óőîäĺ ęóđńîđŕ
				}
			}
		...
		ostringstream playerScore;    // îáú˙âčëč ďĺđĺěĺííóţ
		...
		if (pl.pos != 9) {
			text.setString(playerScore.str() + " - " + word);//çŕäŕĺň ńňđîęó ňĺęńňó
			text.setPosition(1050, 0);
		}
		ostringstream playerHealth;    // îáú˙âčëč ďĺđĺěĺííóţ
		if (pl.pos != 9) {
		text1.setPosition(0, 0);
		playerHealth << pl.health;	//çŕíĺńëč â íĺĺ ÷čńëî çäîđîâü˙, ňî ĺńňü ôîđěčđóĺě ńňđîęó
		text1.setString("ŐÂ:" + playerHealth.str());	//çŕäŕĺě ńňđîęó ňĺęńňó č âűçűâŕĺě ńôîđěčđîâŕííóţ âűřĺ ńňđîęó ěĺňîäîě .str() 
		
		if (pl.life == false) {
				playerHealth << gameTime;
				text1.setCharacterSize(80);
				text1.setPosition(400, 200);
				text1.setString("Âđĺě˙ â ńĺęóíäŕő:" + playerHealth.str());//çŕäŕĺě ńňđîęó ňĺęńňó č âűçűâŕĺě ńôîđěčđîâŕííóţ âűřĺ ńňđîęó ěĺňîäîě .str() 
		}
		}
		if (pl.pos == 9) {
				playerHealth << gameTime;
				...
				text1.setString(playerScore.str() + " - " + word);//çŕäŕĺě ńňđîęó ňĺęńňó č âűçűâŕĺě ńôîđěčđîâŕííóţ âűřĺ ńňđîęó ěĺňîäîě .str() 
				text.setString( " ĘÎÍĹÖ ");
		}	//çŕäŕĺě ńňđîęó ňĺęńňó č âűçűâŕĺě ńôîđěčđîâŕííóţ âűřĺ ńňđîęó ěĺňîäîě .str() 	
		...
		if (pl.textt == 1) {
			text2.setString(" Íóćíî óáčňü âńĺő âđŕăîâ íŕ óđîâíĺ ");
		...
		if (variable == 1) {
			window.draw(text2);
			window.draw(text1);//đčńóţ ýňîň ňĺęńň
			window.draw(text);
			window.draw(monet);
			window.draw(plh);
